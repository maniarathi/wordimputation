...
def create_training(dataset_size, vocab_size, max_sentence_size, min_sentence_size=3):
    ...
    output_filename = 'training_total_' + str(dataset_size) + '_max_' \
...
def get_options():
    ...
    parser = OptionParser()
    parser.add_option('-b', '--maxSentenceSize', type='int', dest='max_sentence_size', help='maximum number of words in the sentence to be kept in the dataset')
    parser.add_option('-s', '--minSentenceSize', type='int', dest='min_sentence_size', help='minimum number of words in the sentence to be kept in the dataset')
    parser.add_option('-d', '--datasetSize', type='int', dest='dataset_size', help='number of sentences to keep in the datasets')
    parser.add_option('-v', '--vocabSize', type='int', dest='vocab_size', help='number of words to keep in the vocabulary')
...
def main():
    ...
    print options
    vocab, last_line_read = create_training(options.dataset_size, options.vocab_size, options.max_sentence_size, options.min_sentence_size)
    last_line_read = create_dev_or_test(vocab, last_line_read, options.dataset_size, options.max_sentence_size, options.min_sentence_size, True)
    create_dev_or_test(vocab, last_line_read, options.dataset_size, options.max_sentence_size, options.min_sentence_size, False)
